...
* 映画名と映画名の対に対する２段階ハッシュの作成プログラムの仕様の定義
	...
	* 入力となるテストデータの作成
	* テストの実施
* ２つの映画の間のtanimoto係数を計算するメソッドの仕様の定義
	* ２つの映画の視聴データ（6人分程度でよい）
	* 特徴的なテストデータで比較する
		* 完全に視聴履歴が一致している場合
		* 完全に視聴履歴が不一致の場合
		* 50%が一致している場合
		* tanimoto係数による比較が不適切な場合（一方が全部0のものを排除する）
	* テストの実施
...
* メタ文字 \w (アルファベット、数字、アンダスコア（ _ ）のどれかの1文字) を使って次のように書くこともできる
...
* ちょうど８文字でマッチするようにするには、文字列の先頭と文字列の末尾の条件を入れてあげる
...
メタ文字として認識されて、本来は排除したい文字列までマッチしてしまう
...
```ruby
p "脳は成長するため、喜ぶために壁にぶつかることを求めているんだよ。".split(/ため/)
```

* "ため"という文字列を無くなっている

```ruby
["脳は成長する", "、喜ぶ", "に壁にぶつかることを求めているんだよ。"]
```
...
```ruby
 ["脳は成長する", "ため", "、喜ぶ", "ため", "に壁にぶつかることを求めているんだよ。"]
```
...
```ruby
["", "脳は成長するため、喜ぶため", "に壁にぶつかることを求めているんだよ。"]
```
...
```ruby
p "脳は成長するため、喜ぶために壁にぶつかることを求めているんだよ。".split(/(.+?ため)/).reject{|x|x==""}
```
...
```
イカを見てみろよ、
いろいろな魚が泳いでいる所でも、周りを気にせず、常にイカイカしているだろ。
イカは「自分」を持っている、
常に自分の心に耳を傾け、自分で考え決断し、行動しているんだよ。
```
